...
import { verifyCredentials, createAdminSession } from "./_core/auth";
...
const addFavoriteSchema = z.object({
  ...
  facilityState: z.string().max(50).optional(),
  facilityZip: z.string().max(20).optional(),
  ...
  auth: router({
    ...
    login: publicProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        const valid = verifyCredentials(input.email, input.password);
        if (!valid) {
    ...
    add: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        const result = await addUserFavorite({
          ...
          facilityCity: input.facilityCity || null,
          facilityState: input.facilityState || null,
          facilityZip: input.facilityZip || null,
