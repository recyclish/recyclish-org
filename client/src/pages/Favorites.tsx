...
export default function Favorites() {
  ...
  // Convert favorites to Shelter format using structured address fields
  // Falls back to comma-splitting the legacy facilityAddress only when structured fields are absent
  const sheltersFromFavorites: Shelter[] = (favorites || []).map(fav => {
    // Use structured fields if available; otherwise fall back to parsing the combined string
    const city = fav.facilityCity || fav.facilityAddress.split(',')[1]?.trim() || "";
    const state = fav.facilityState || fav.facilityAddress.split(',')[2]?.trim().split(' ')[0] || "";
    const zip = fav.facilityZip || fav.facilityAddress.split(',')[2]?.trim().split(' ')[1] || "";
    const street = fav.facilityAddress.split(',')[0] || fav.facilityAddress;
    ...
    return {
      ...
      shelterType: fav.facilityCategory || "Recycling Center",
      speciesServed: fav.facilityFeedstock ? fav.facilityFeedstock.split(',') : [],
      isNoKill: false,
      verified: false,
    ...
    <div className="min-h-screen flex flex-col bg-cream font-body selection:bg-terracotta/20 selection:text-terracotta">
      ...
      <section className="bg-ocean text-cream py-16 px-6 relative overflow-hidden">
        <div className="absolute top-1/2 right-10 -translate-y-1/2 opacity-[0.03] select-none pointer-events-none hidden lg:block">
          <span className="text-[15vw] font-display font-black leading-none tracking-tighter uppercase italic">Saved</span>
        </div>
          ...
          >
            ...
            <div className="flex items-center gap-4 mb-6">
              ...
              <h1 className="font-display text-5xl md:text-7xl font-bold leading-tight tracking-tight">
                Your <br />
                <span className="text-terracotta italic underline decoration-terracotta/30 underline-offset-8">Saved Centers</span>
              </h1>
            </div>

            <p className="text-xl text-cream/60 font-medium leading-relaxed max-w-2xl">
              A personalized list of recycling centers and drop-off locations you've bookmarked.
            </p>
          </motion.div>
        </div>
      </section>
      ...
      <main className="flex-1 container py-20 px-6 relative overflow-hidden">
        ...
        {isLoading ? (
          ...
          <div className="text-center py-32 bg-white/40 backdrop-blur-xl rounded-[4rem] border border-ocean/5 max-w-4xl mx-auto">
            ...
            <h3 className="font-display text-4xl font-bold text-ocean mb-4">No Saved Centers Yet</h3>
            <p className="text-ocean/40 font-medium text-lg mb-12 max-w-sm mx-auto">
              Browse the directory and bookmark recycling centers to build your personal list.
            </p>
          ...
          >
            <div className="flex items-center justify-between mb-12">
              <div className="space-y-1">
                ...
                <p className="text-[10px] font-label uppercase tracking-widest text-terracotta font-black">Saved Recycling Centers</p>
              </div>
            </div>
